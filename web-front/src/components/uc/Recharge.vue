...
<!--                </p>-->
            ...
            <div class="action-content" style="width: 200px;">
              <div class="action-body">
                <p class="acb-p1">{{$t('uc.finance.recharge.amount')}}</p>
                <Form ref="formValidate" :label-width="0">
                  <FormItem>
                    <Input v-model="depositNum" :placeholder="$t('uc.finance.recharge.amount')"></Input>
                  </FormItem>
                </Form>
                <Button type="primary" size="large" long @click="depositAdd">{{$t('uc.finance.withdraw.submit')}}</Button>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    depositAdd(){
      if(this.depositNum == "" || this.depositNum<0) {
        this.$Message.error(this.$t("otc.publishad.warning1"));
        return;
      }
      let param = {};
      param["unit"] = this.qrcode.unit;
      param["amount"] = this.depositNum;

      this.$http
          .post(this.host + "/uc/asset/wallet/deposit_add", param)
          .then(response => {
            var resp = response.body;
            if (resp.code == 0) {
              this.depositNum = 0;
              this.$Message.success(this.$t("exchange.success"));
            } else {
              this.$Message.error(resp.message);
            }
          });
