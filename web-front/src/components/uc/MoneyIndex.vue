      ...
      <div class="bill_box rightarea padding-right-clear" style="padding-left: 5px;">
        <div class="shaow">
          <div class="money_table" style="text-align: left;">
            <div style="width: 100%;">
              <div style="letter-spacing:1px;padding-top: 5px;">
                <p style="font-size:12px;color:#828ea1;text-align: left;">{{$t('uc.finance.money.totalassets')}}</p>
                <!-- <div style="display: flex;"> -->
                  <span style="font-size: 15px;color:#D8E1EB; text-align: left;">${{totalUSDT}}</span>
                <span style="font-size:10px;color:#828ea1;text-align: left;"> ≈ ¥{{totalCny}}</span>
                <!-- </div> -->
              </div>
              <div style="display: flex; margin-top: 10px;width: 100%;">
                <Button @click="toQuickExchange" type="warning"
                style="padding: 2px 15px;margin-right:10px;background-color:#f0a70a;border-color:#f0a70a; height: 30px;">{{$t('uc.finance.record.quickExchange')}}</Button>
              <Input style="display: block; width: 80%;" class="search" search :placeholder="$t('common.searchplaceholder')"
                @on-change="seachInputChange" v-model="searchKey" />
              </div>
              

            </div>
            <div class="xs_table" style="">
              <Table :no-data-text="$t('common.nodata')" :columns="tableColumnsMoney" :data="tableMoneyShow"
              :loading="loading" :disabled-hover="true" style="width: 200%;"></Table>
            </div>
...
<!--              <Button @click="toQuickExchange" type="warning"
                style="padding: 6px 30px;margin-right:30px;background-color:#f0a70a;border-color:#f0a70a;float:right;">{{$t('uc.finance.record.quickExchange')}}</Button> -->
            </div>
            <Table :no-data-text="$t('common.nodata')" :columns="tableColumnsMoney" :data="tableMoneyShow"
              :loading="loading" :disabled-hover="true"></Table>
          </div>
        </div>
      </div>
    </div>
    ...
    <Modal v-model="transModal" @on-ok="confrimTrans" :title="$t('uc.finance.money.onkeytrans')">
      <p>{{$t('uc.finance.record.chooseTransCoinUnit')}}：
        <Select v-if="xsShow" v-model="toUnit" style="width: 70%;" :placeholder="$t('common.pleaseselect')">
          <Option v-for="item in transList" :value="item" :key="item">{{ item }}</Option>
        </Select>
        <Select v-else v-model="toUnit" style="width: 400px;" :placeholder="$t('common.pleaseselect')">
          <Option v-for="item in transList" :value="item" :key="item">{{ item }}</Option>
        </Select>
      </p>
      <p style="margin-top: 15px;">{{$t('uc.finance.record.inputTransAmount')}}：
        <InputNumber v-if="xsShow" style="width: 70%;" type="text" v-model="transAmount"
          :placeholder="$t('uc.finance.money.matchtip2')"></InputNumber>
        <InputNumber v-else style="width: 400px;" type="text" v-model="transAmount"
          :placeholder="$t('uc.finance.money.matchtip2')"></InputNumber>
      </p>
    </Modal>
    ...
    <Modal v-model="quickExchangeModal" @on-ok="confrimExchange" :title="$t('uc.finance.record.quickExchange')">
      ...
      <p style="margin-top: 15px;">{{$t('uc.finance.record.from')}}：
        <Select v-if="xsShow" v-model="fromExchangeCoin" style="width: 90%;" :placeholder="$t('common.pleaseselect')">
          <Option v-for="item in fromCoinList" :value="item" :key="item">{{ item }}</Option>
        </Select>
        <Select v-else v-model="fromExchangeCoin" style="width: 450px;" :placeholder="$t('common.pleaseselect')">
          <Option v-for="item in fromCoinList" :value="item" :key="item">{{ item }}</Option>
        </Select>
      </p>
      <p style="margin-top: 15px;">{{$t('uc.finance.record.to')}}：
        <Select v-if="xsShow" v-model="toExchangeCoin" style="width: 90%;" :placeholder="$t('common.pleaseselect')">
          <Option v-for="item in toCoinList" :value="item" :key="item">{{ item }}</Option>
        </Select>
        <Select v-else v-model="toExchangeCoin" style="width: 450px;" :placeholder="$t('common.pleaseselect')">
          <Option v-for="item in toCoinList" :value="item" :key="item">{{ item }}</Option>
        </Select>
      </p>
      <p style="margin-top: 15px;">{{$t('uc.finance.record.inputexchangeamount')}}:
        <InputNumber v-if="xsShow" style="width: 56%;" type="text" v-model="exchangeAmount"
          :placeholder="$t('uc.finance.record.inputexchangeamount')"></InputNumber>
        <InputNumber v-else style="width: 330px;" type="text" v-model="exchangeAmount"
          :placeholder="$t('uc.finance.record.inputexchangeamount')"></InputNumber>
      ...
      <p style="margin-top: 15px;">{{$t('uc.finance.record.predictAmount')}}:
        <InputNumber v-if="xsShow" style="width: 56%;" disabled type="text" v-model="predictAmount"></InputNumber>
        <InputNumber v-else style="width: 330px;" disabled type="text" v-model="predictAmount"></InputNumber>
      ...
      <p style="margin-top: 15px;">{{$t('uc.finance.record.inputexchangepasswd')}}:
        <Input v-if="xsShow" style="width: 74%;" type="password" v-model="exchangePassword"
          :placeholder="$t('uc.finance.record.inputexchangepasswd')"></Input>
        <Input v-else style="width: 400px;" type="password" v-model="exchangePassword"
          :placeholder="$t('uc.finance.record.inputexchangepasswd')"></Input>
      </p>
    </Modal>
  </div>
