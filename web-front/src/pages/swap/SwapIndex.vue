  ...
  <div class="container swap" :class="skin">
    ...
    <div v-if="xsShow" class="tabBar">
      <Tabs>
        ...
        <Tab-pane label="K线图">
          <div class="center">
            <div class="symbol" style="flex-wrap: wrap;">
                ...
                <div v-if="entrustType === 1" class="open">
                  ...
                  <Form style="width: 94%;margin-left: 3%;margin-top:10px;">
                    <FormItem style="margin-bottom: 10px;" v-if="entrustOrderType == '1'">
                      ...
                      <Input
                        ...
                        @on-keyup="form.entrustPrice = form.entrustPrice.replace(/[^\d^\.]+/g, '')"
                      ></Input>
                    ...
                    <FormItem style="margin-bottom: 10px;" v-if="entrustOrderType == '2'">
                      ...
                      <Input
                        ...
                        @on-keyup="form.triggerPrice = form.triggerPrice.replace(/[^\d^\.]+/g, '')"
                      ></Input>
                      <label class="after">{{ currentCoin.base }}</label>
                    </FormItem>
                    ...
                    <FormItem style="margin-bottom: 10px;" v-if="entrustOrderType == '2'">
                      ...
                      <Input
                        v-model="form.entrustPrice"
                        :placeholder="$t('swap.triggleplaceholder')"
                        @on-keyup="form.entrustPrice = form.entrustPrice.replace(/[^\d^\.]+/g, '')"
                      ></Input>
                      <label class="after">{{ currentCoin.base }}</label>
                    </FormItem>
                    ...
                    <FormItem style="margin-bottom: 10px;">
                      ...
                      <Input
                        v-model="form.volume"
                        v-if="sliderOpenPercent == 0"
                        @on-keyup="form.volume = form.volume.replace(/\D/g, '')"
                      ></Input>
                      <Input
                        class="market-price"
                        :readonly="true"
                        v-if="sliderOpenPercent != 0"
                        @on-focus="onOpenAmountFocus"
                        :value="sliderOpenPercent + ' %'"
                      ></Input>
                      <label class="after" v-if="sliderOpenPercent == 0">{{ $t('swap.shareNumber') }}</label>
                      <span style="position:absolute;top:32px;right:0px;color: #7c7f82;"
                        >1{{ $t('swap.shareNumber') }} = {{ contractCoinInfo.shareNumber }}USDT</span
                      >
                    </FormItem>
                    <div class="split-panel" v-if="entrustOrderType != '2'">
                      <div :class="{ selected: Number(sliderOpenPercent) === 25 }" @click="onChangeOpenPercent(25)">
                        25%
                      </div>
                      <div :class="{ selected: Number(sliderOpenPercent) === 50 }" @click="onChangeOpenPercent(50)">
                        50%
                      </div>
                      <div :class="{ selected: Number(sliderOpenPercent) === 75 }" @click="onChangeOpenPercent(75)">
                        75%
                      </div>
                      <div :class="{ selected: Number(sliderOpenPercent) === 100 }" @click="onChangeOpenPercent(100)">
                        100%
                      </div>
                    </div>
                  </Form>
                  <div class="open-tips">
                    <div>
                      ...
                      ><span class="num">{{ avaOpenBuy }} {{ $t('swap.shareNumber') }}</span>
                    </div>
                    <div>
                      <span class="red">{{ $t('swap.candown') }}:</span
                      ><span class="num">{{ avaOpenSell }} {{ $t('swap.shareNumber') }}</span>
                    </div>
                  </div>
                  <div class="operate" v-if="isLogin">
                    <Button type="primary" shape="circle" class="open-up" @click="onOpen(0)">{{
                      $t('swap.openup')
                    }}</Button>
                    <Button type="primary" shape="circle" class="open-down" @click="onOpen(1)">{{
                      $t('swap.opendown')
                    }}</Button>
                  </div>
                ...
                <div v-else class="open">
                  ...
                  <Form style="width: 94%;margin-left: 3%;margin-top:10px;">
                    ...
                    <FormItem style="margin-bottom: 10px;" v-if="entrustOrderType == '2'">
                      ...
                      <Input
                        v-model="form.triggerPrice"
                        @on-keyup="form.trigglePrice = form.triggerPrice.replace(/[^\d^\.]+/g, '')"
                      ></Input>
                      <label class="after">{{ currentCoin.base }}</label>
                    </FormItem>
                    ...
                    <div class="split-panel" v-if="entrustOrderType != '2'">
                      <div :class="{ selected: Number(sliderClosePercent) === 25 }" @click="onChangeClosePercent(25)">
                        25%
                      </div>
                      <div :class="{ selected: Number(sliderClosePercent) === 50 }" @click="onChangeClosePercent(50)">
                        50%
                      </div>
                      <div :class="{ selected: Number(sliderClosePercent) === 75 }" @click="onChangeClosePercent(75)">
                        75%
                      </div>
                  ...
                  <div class="open-tips">
                    <div>
                      ...
                      ><span class="num">{{ contractWalletInfo.usdtBuyPosition }} {{ $t('swap.shareNumber') }}</span>
                    </div>
                    <div>
                      <span class="green">{{ $t('swap.cancloseup') }}:</span
                      ><span class="num">{{ contractWalletInfo.usdtSellPosition }} {{ $t('swap.shareNumber') }}</span>
                    </div>
                  </div>
                  <div class="operate" v-if="isLogin">
                    <Button type="primary" shape="circle" class="open-down" @click="onClose(1)">{{
                      $t('swap.closedown')
                    }}</Button>
                    <Button type="primary" shape="circle" class="open-up" @click="onClose(0)">{{
                      $t('swap.closeup')
                    }}</Button>
                  </div>
                  <div class="operate-login" v-if="!isLogin" style="width: 94%; margin-left:3%;">
        ...
        <Tab-pane label="最新交易">
          <div class="left plate-wrap" style="position:relative; flex: 0 0 13%;">
            <div
              style="background-color: #192330;height:40px;line-height:40px;padding-left:5px;color:#61688A;font-size: 13px;"
            >
              <span>{{ $t('swap.latestdeal') }}</span>
            </div>
            <div class="trade-wrap">
              <Table height="472" :columns="trade.columns" :data="trade.rows" :no-data-text="$t('common.nodata')">
              </Table>
            </div>
            <!-- 我的合约账户 -->
            <div class="order" style="margin-top: 5px;min-height: 320px;background-color: #192330;color:#61688A;">
              <div
                style="height:32px;line-height:32px;padding-left:10px;border-bottom: 1px solid #27313e;font-size: 13px;"
              >
              ...
              <div class="table swap-my-account">
                <div class="account-item" style="margin-top: 20px;">
                  ...
                  <div class="mode" style="width: 65%;">
                    <Button @click="showMarginModeModal()" v-if="marginMode === '1'" size="small">{{
                      $t('swap.marginMode1')
                    }}</Button>
                    <Button @click="showMarginModeModal()" v-if="marginMode === '0'" size="small">{{
                      $t('swap.marginMode2')
                    }}</Button>
                  </div>
                </div>
                <div
                  class="account-item"
                  style="margin-top: 20px;height:42px;padding-bottom:5px;border-bottom: 1px dashed #27313e;"
                >
                  ...
                  <div class="margin" style="width: 65%;display: flex;justify-content: space-between;">
                    <Button
                      ...
                      size="small"
                      style="flex: 0 0 47%;border:none;background: #4db872;"
                      >{{ buyLeverage }}X{{ $t('swap.up') }}</Button
                    >
                    <Button
                      @click="showAdjustLeverage(2)"
                      size="small"
                      style="flex: 0 0 47%;border:none;background: #ee6560;"
                      >{{ sellLeverage }}X{{ $t('swap.down') }}</Button
                    >
                  </div>
                </div>

                <div class="account-item" style="margin-top:10px;">
                  <div>{{ $t('swap.accountquanyi') }}</div>
                  <div>
                    <span>{{ totalQuanyi | fixed4 }}</span>
                  </div>
                </div>
                <div class="account-item">
                  <div>{{ $t('swap.profitandloss') }}</div>
                  <div>
                    <span>{{ profitAndLoss | fixed4 }}</span>
                  </div>
                </div>
                <div class="account-item">
                  <div>{{ $t('swap.principalAmount') }}</div>
                  <div>
                    <span>{{ contractWalletInfo.usdtBalance | fixed4 }}</span>
                  </div>
                </div>
                <div class="account-item">
                  ...
                  <div>
                    <span>{{
                      (contractWalletInfo.usdtBuyPrincipalAmount + contractWalletInfo.usdtSellPrincipalAmount) | fixed4
                    }}</span>
                  </div>
                </div>
                <div class="account-item">
                  ...
                  <div>
                    <span>{{ contractWalletInfo.usdtFrozenBalance | fixed4 }}</span>
                  </div>
                </div>
                <div class="account-item">
                  <div>{{ $t('swap.principalRate') }}</div>
                  <div>
                    <span>{{ assetRate | percent }} %</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="order" style="background: #999;margin-right: 5px;">
              <div class="order-handler">
                ...
                <span
                  ...
                  :class="{ active: selectedOrder === 'currentEntrustOrders' }"
                  >{{ $t('swap.curdelegation') }}</span
                >
                <span
                  @click="changeOrder('historyEntrustOrders')"
                  :class="{ active: selectedOrder === 'historyEntrustOrders' }"
              ...
              <Form style="width: 94%;margin-left: 3%;margin-top:10px;">
                ...
                <FormItem style="margin-bottom: 10px;">
                  ...
                  <Input
                    ...
                    v-if="sliderClosePercent != 0"
                    @on-focus="onCloseAmountFocus"
                    :value="sliderClosePercent + ' %'"
                  ></Input>
                  <label class="after" v-if="sliderClosePercent == 0">{{ $t('swap.shareNumber') }}</label>
                  <span style="position:absolute;top:32px;right:0px;color: #7c7f82;"
                    >1{{ $t('swap.shareNumber') }} = {{ contractCoinInfo.shareNumber }}USDT</span
                  >
                </FormItem>
      ...
      <div class="leverage">
        ...
        <div class="leverage-panel" v-if="leverageType == 1 && changeLeverageType == 1">
          <Button
            ...
            :class="{ leverageSelect: Number(item) === leverageAdjustVal }"
            >{{ item }}X</Button
          >
        </div>
...
export default {
        ...
        columns: [
          {
            ...
            renderHeader: (h, params) => {
              const title = self.$t('swap.price') + '(' + self.currentCoin.base + ')'
              return h('span', {}, title)
            },
          },
          {
        ...
        columns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              params.row.amount != 0 && (str = h('span', {}, params.row.amount.toFixed(4)))
              return str
            },
            renderHeader: (h, params) => {
              const title = self.$t('swap.num') + '(' + self.currentCoin.coin + ')'
          ...
          },
          {
            ...
            renderHeader: (h, params) => {
              const title = self.$t('swap.total') + '(' + self.currentCoin.coin + ')'
              return h('span', {}, title)
            },
          },
          {
  ...
  computed: {
    ...
    profitAndLoss: function() {
      ...
      if (this.contractWalletInfo.usdtBuyPrice > 0) {
        buyPL =
          ...
          (this.contractWalletInfo.usdtBuyPosition + this.contractWalletInfo.usdtFrozenBuyPosition) *
      ...
      if (this.contractWalletInfo.usdtSellPrice > 0) {
        sellPL =
          ...
          (this.contractWalletInfo.usdtSellPosition + this.contractWalletInfo.usdtFrozenSellPosition) *
          this.contractWalletInfo.usdtShareNumber
      }
    ...
    avaOpenBuy: function() {
      ...
      if (this.contractWalletInfo.usdtPattern == 'FIXED') {
        ...
        if (lossandp >= 0) {
          ...
          return parseInt(
            (this.buyLeverage * (this.contractWalletInfo.usdtBalance + lossandp)) / this.contractCoinInfo.shareNumber
          )
        }
      }
    },
    avaOpenSell: function() {
      ...
      if (this.contractWalletInfo.usdtPattern == 'FIXED') {
        ...
        if (lossandp >= 0) {
          ...
          return parseInt(
            (this.sellLeverage * (this.contractWalletInfo.usdtBalance - lossandp)) / this.contractCoinInfo.shareNumber
          )
        }
      }
    },
    positionList: function(){
      ...
      if(this.contractWalletInfo.usdtBuyPosition > 0 || this.contractWalletInfo.usdtFrozenBuyPosition > 0) {
        ...
        buyMinNeedPrinc = (this.contractWalletInfo.usdtBuyPosition + this.contractWalletInfo.usdtFrozenBuyPosition) * this.contractWalletInfo.usdtShareNumber / this.contractWalletInfo.usdtBuyLeverage;
        buyCloseFee = (this.contractWalletInfo.usdtBuyPosition + this.contractWalletInfo.usdtFrozenBuyPosition) * this.contractWalletInfo.usdtShareNumber * this.contractCoinInfo.closeFee;
        ...
        if(this.contractWalletInfo.usdtPattern == "FIXED") {
          mRatio = (buyPl + this.contractWalletInfo.usdtBuyPrincipalAmount - buyCloseFee) / buyMinNeedPrinc;
          this.$set(this.currentPositionList[0], "mRatio", mRatio);
      ...
      if(this.contractWalletInfo.usdtSellPosition > 0 || this.contractWalletInfo.usdtFrozenSellPosition > 0) {
        ...
        sellMinNeedPrinc = (this.contractWalletInfo.usdtSellPosition + this.contractWalletInfo.usdtFrozenSellPosition) * this.contractWalletInfo.usdtShareNumber / this.contractWalletInfo.usdtSellLeverage;
        sellCloseFee = (this.contractWalletInfo.usdtSellPosition + this.contractWalletInfo.usdtFrozenSellPosition) * this.contractWalletInfo.usdtShareNumber * this.contractCoinInfo.closeFee;
        ...
        if(this.contractWalletInfo.usdtPattern == "FIXED") {
          mRatio = (sellPl + this.contractWalletInfo.usdtSellPrincipalAmount - sellCloseFee) / sellMinNeedPrinc;
          this.$set(this.currentPositionList[temIndex], "mRatio", mRatio);
      ...
      if(this.contractWalletInfo.usdtPattern == "CROSSED") {
        mRatio = (buyPl + sellPl + this.contractWalletInfo.usdtBuyPrincipalAmount + this.contractWalletInfo.usdtSellPrincipalAmount
            + this.contractWalletInfo.usdtBalance + this.contractWalletInfo.usdtFrozenBalance - buyCloseFee - sellCloseFee) / ( buyMinNeedPrinc + sellMinNeedPrinc );
        this.$set(this.currentPositionList[0], "mRatio", mRatio);
        if(temIndex === 1){
          this.$set(this.currentPositionList[temIndex], "mRatio", mRatio);
        }
  ...
  methods: {
          ...
          if (resp != null) {
            ...
            if (this.contractWalletInfo.usdtSellPosition > 0 || this.contractWalletInfo.usdtFrozenSellPosition > 0) {
              ...
              var tItem = {
                ...
                plRatio:
                  this.contractWalletInfo.usdtTotalProfitAndLoss / this.contractWalletInfo.usdtSellPrincipalAmount,
                mRatio:
    ...
    getPlate(str = '') {
      ...
      this.$http.post(this.host + this.api.swap.platemini, params).then(response => {
        this.plate.askRows = []
        this.plate.bidRows = []
    ...
    getPlateFull() {
      ...
      var params = {}
      params['symbol'] = this.currentCoin.symbol
      this.$http.post(this.host + this.api.swap.platefull, params).then(response => {
        var resp = response.body
        this.fullTrade = resp
        resp.skin = this.skin
    ...
    getTrade() {
      ...
      params['symbol'] = this.currentCoin.symbol
      params['size'] = 20
      this.$http.post(this.host + this.api.swap.trade, params).then(response => {
        this.trade.rows = []
        var resp = response.body
        for (var i = 0; i < resp.length; i++) {
    ...
    successCallback(that, stompClient) {
      that.datafeed = new Datafeeds.WebsockFeed(that.host + '/swap', that.currentCoin, stompClient, that.baseCoinScale)
      that.getKline()
      //订阅价格变化消息
      ...
      //订阅盘口消息
      stompClient.subscribe('/topic/swap/trade-plate/' + that.currentCoin.symbol, function(msg) {
        var resp = JSON.parse(msg.body)
    ...
    cancel(eid) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          this.$http.post(this.host + this.api.swap.cancel, params).then(response => {
            var resp = response.body
            if (resp.code == 0) {
              this.$Notice.success({
    ...
    changeMarginMode(val) {
      ...
      this.$http.post(this.host + this.api.swap.switchPattern, params).then(response => {
        var resp = response.body
        if (resp.code == 0) {
    ...
    adjustLeverage() {
      ...
      this.$http.post(this.host + this.api.swap.modifyLeverage, params).then(response => {
        var resp = response.body
        if (resp.code == 0) {
          this.$Notice.success({
    ...
    onOpen(direction) {
      ...
      this.$Spin.show()
      var that = this
      this.$http.post(this.host + this.api.swap.open, params).then(response => {
        var resp = response.body
        if (resp.code == 0) {
          this.$Notice.success({
            title: that.$t('swap.tip'),
    ...
    onClose(direction) {
      ...
      this.$Spin.show()
      var that = this
      this.$http.post(this.host + this.api.swap.close, params).then(response => {
        var resp = response.body
        if (resp.code == 0) {
          this.$Notice.success({
            title: that.$t('swap.tip'),
    ...
    getCurrentEntrustOrders() {
      ...
      this.$http.post(this.host + this.api.swap.getEntrustList, params).then(response => {
        var resp = response.body
    ...
    getHistoryEntrustOrders() {
      ...
      this.$http.post(this.host + this.api.swap.getHistoryEntrustList, params).then(response => {
        var resp = response.body
        if (resp.content.length > 0) {
