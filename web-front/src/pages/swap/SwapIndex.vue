            ...
            <div v-if="entrustType===1" class="open">
              ...
              <Form style="width: 94%;margin-left: 3%;margin-top:10px;">
                  ...
                  <FormItem style="margin-bottom: 10px;" >
                    ...
                    <span style="position:absolute;top:32px;right:0px;color: #7c7f82;">1{{$t('swap.shareNumber')}} = {{contractCoinInfo.shareNumber}}{{ contractCoinInfo.coinSymbol }}</span>
                  </FormItem>
  ...
  computed: {
    ...
    avaOpenBuy: function(){ // 可开多
      ...
      if(this.contractWalletInfo.usdtPattern == "FIXED") { // 逐仓，直接usdt余额即可
        return parseInt(this.buyLeverage * this.contractWalletInfo.usdtBalance / (this.contractCoinInfo.shareNumber*this.currentCoin.price));
      }else{ // 全仓模式
        // 计算多仓盈亏
        var buyPL = 0;
        if(this.contractWalletInfo.usdtBuyPrice > 0){
          ...
          // buyPL = (this.currentCoin.price / this.contractWalletInfo.usdtBuyPrice -1) * (this.contractWalletInfo.usdtBuyPosition + this.contractWalletInfo.usdtFrozenBuyPosition) * this.contractWalletInfo.usdtShareNumber;
          buyPL = (this.currentCoin.price - this.contractWalletInfo.usdtBuyPrice) * (this.contractWalletInfo.usdtBuyPosition + this.contractWalletInfo.usdtFrozenBuyPosition) * this.contractWalletInfo.usdtShareNumber;
        }
        // 计算空仓盈亏
        var sellPL = 0;
        if(this.contractWalletInfo.usdtSellPrice > 0){
          // 空仓盈亏（1 - 当前价格 / 开仓均价）* （可用空单仓位 + 冻结空单仓位） * 合约面值
          sellPL = (this.contractWalletInfo.usdtSellPrice - this.currentCoin.price) * (this.contractWalletInfo.usdtSellPosition + this.contractWalletInfo.usdtFrozenSellPosition) * this.contractWalletInfo.usdtShareNumber;
        }
        ...
        if(lossandp >= 0) {
          ...
          return parseInt(this.buyLeverage * (this.contractWalletInfo.usdtBalance + lossandp) / (this.contractCoinInfo.shareNumber*this.currentCoin.price));
        }
      }
    },
    avaOpenSell: function(){ // 可开空
      ...
      // 计算可开多/空
      if(this.contractWalletInfo.usdtPattern == "FIXED") { // 逐仓，直接usdt余额即可
        return parseInt(this.sellLeverage * this.contractWalletInfo.usdtBalance / (this.contractCoinInfo.shareNumber*this.currentCoin.price));
      }else{
        // 计算可用
        var buyPL = 0;
        if(this.contractWalletInfo.usdtBuyPrice > 0){
        ...
        if(lossandp >= 0) {
          ...
          return parseInt(this.sellLeverage * (this.contractWalletInfo.usdtBalance - lossandp) / (this.contractCoinInfo.shareNumber*this.currentCoin.price));
        }
      }
    },
