<template>
  ...
  <div class="option">
    <div class="ctc_container">
      ...
      <div class="main">
        <!-- tab切换 -->
        <div class="option-tab bz_container">
          <div
            :class="{ 'option-tab-item': true, 'option-tab-current': item.symbol == currentCoinSymbol }"
            class="option-tab-item"
            v-for="item in coinList"
            @click="changeCoin(item.symbol)"
            :key="item.id"
          >
            {{ item.symbol }} {{ $t('option.title') }}
          </div>
        </div>
        <div class="trade_panel" style="width: 100%;min-height: 470px;">
          <div class="trade_bd_ctc">
            <div class="panel panel_buy">
              <div class="bd bd_limited">
                <!-- content -->
                <div class="result-panel bz_container">
                  <!-- 标题tab切换 -->
                  <div class="result-panel-item">
                    <span
                      :class="{ 'title-switch': true, 'switch-current': historyTab == 1 }"
                      @click="historyClick(1)"
                      >{{ $t('option.history') }}</span
                    >
                    <span
                      :class="{ 'title-switch': true, 'switch-current': historyTab == 2 }"
                      @click="historyClick(2)"
                      >{{ $t('option.kline') }}</span
                    >
                  </div>
                  <!-- part1 -->
                  <div
                    ...
                    v-show="historyTab == 2"
                    style="height: 240px;position: relative;overflow: hidden;"
                  >
                    <div id="kline_container"></div>
                  </div>
                  <!-- part2 -->
                  <div v-show="historyTab == 1" class="result-content">
                    <!-- -----------item--------- -->
                    <div class="result-item" v-for="item in historyList" :key="item.id">
                      <div class="item-title" v-if="width > 768">
                        <span>{{ $t('option.seriers') }}</span
                        ><span>{{ item.optionNo }}</span
                        ><span>{{ $t('option.period') }}</span>
                      </div>
                      <div class="item-body">
                        <Tooltip placement="bottom">
                          ...
                          <div slot="content">
                            <p class="tip-item">
                              <span>{{ $t('option.opentime') }}：</span>{{ item.openTime | dateFormat(that) }}
                            </p>
                            <p class="tip-item">
                              <span>{{ $t('option.closetime') }}：</span>{{ item.closeTime | dateFormat(that) }}
                            </p>
                            <p class="tip-item">
                              <span>{{ $t('option.openprice1') }}：</span>{{ item.openPrice }} USDT
                            </p>
                            <p class="tip-item">
                              <span>{{ $t('option.closeprice') }}：</span>{{ item.closePrice }} USDT
                            </p>
                            <p class="tip-item">
                              <span>{{ $t('option.buyupreward') }}：</span>{{ item.totalBuy }} USDT
                            </p>
                            <p class="tip-item">
                              <span>{{ $t('option.buydownreward') }}：</span>{{ item.totalSell }} USDT
                            </p>
                          </div>
                        </Tooltip>
                      </div>
                ...
                <!-- 统计 -->
                <div class="last-period bz_container" v-if="openingOption.id == 0">
                ...
                <div class="last-period bz_container" v-if="openingOption.id != 0">
                  ...
                  >
                    ...
                    <Tooltip placement="top-start">
                      ...
                      <div slot="content">
                        {{ (openingOption.createTime + currentCoin.openTimeGap * 1000) | dateFormat(that) }} ~
                        {{
                          (openingOption.createTime + currentCoin.openTimeGap * 1000 + currentCoin.closeTimeGap * 1000)
                            | dateFormat(that)
                        }}
                      </div>
                    </Tooltip>
                  ...
                  <div class="period-content">
                    <div class="content-item color_green">
                      <span
                        >{{ $t('option.buyupreward') }}: <b>{{ openingOption.totalBuy }}</b> USDT</span
                      >
                    </div>
                    <div class="content-item color_red">
                      <span
                        >{{ $t('option.buydownreward') }}: <b>{{ openingOption.totalSell }}</b> USDT</span
                      >
                    </div>
                    <div class="content-item color_gold" v-if="isLogin">
                      <span
                        >{{ $t('option.myamount') }}: <b>{{ myOpeningOption.betAmount }}</b> USDT</span
                      >
                      &nbsp;&nbsp;
                      <span class="direction color_green" v-if="myOpeningOption.direction == 'BUY'">{{
                        $t('option.buyup')
                      }}</span>
                      <span class="direction color_red" v-if="myOpeningOption.direction == 'SELL'">{{
                        $t('option.buydown')
                      }}</span>
                    </div>
                  ...
                  <div style="float:right;height:40px;line-height:40px;padding-right:10px;">
                    <span
                      ...
                      style="letter-spacing:2px;display: inline-block;height:20px;line-height:20px;border-radius: 3px;padding: 0 5px;color:#FFF;"
                      >{{ priceChange }}</span
                    >
                  </div>
                  <div style="color:#a5a5a5;width:170px;float: right;height:40px;text-align: center;">
                    ...
                    >
                      {{ $t('option.openprice') }}：{{ openPrice | fixed2 }} USDT
                    </div>
                    <div style="height:16px;line-height: 16px;font-size: 6px;margin-left:5px;text-align: left;">
                      {{ $t('option.currentprice') }}：<span
                        :class="{ color_green: currentPrice > openPrice, color_red: currentPrice < openPrice }"
                        >{{ currentPrice | fixed2 }} USDT</span
                      >
                    </div>
                  </div>
                  <div
                    style="width:200px;float: right;height:40px;line-height: 36px;text-align: right;padding-right: 10px;"
                  >
                ...
                <div class="current-period bz_container">
                  <div class="content">
                    <div class="period-title">
                      {{ $t('option.seriers') }} {{ startingOption.optionNo }} {{ $t('option.period') }}
                    </div>
                    <div class="period-time">
                      <div style="margin-top:10px;font-size:12px;">
                        {{ $t('option.currentoption') }}：{{
                          (startingOption.createTime + currentCoin.openTimeGap * 1000) | dateFormat(that)
                        }}
                        ~
                        {{
                          (startingOption.createTime +
                            currentCoin.openTimeGap * 1000 +
                            currentCoin.closeTimeGap * 1000)
                            | dateFormat(that)
                        }}
                      </div>
                    </div>
                    <div
                      class="period-reward color_green"
                  ...
                  <Form style="width: 100%;margin-top: 20px;">
                    <FormItem class="buy-input">
                      <div class="quantity-group">
                        <div
                          v-for="item in currentCoinAmountList"
                ...
                <!-- 操作 -->
                <div style="margin-top: 20px;" class="bz_container">
                  <div style="width: 50%;float:left;text-align: center;">
                    <Button v-if="isLogin" class="bg-green" style="width: 80%;" size="large" @click="addClick(0)">{{
                      $t('option.buyup')
                    }}</Button>
                  ...
                  <div style="width: 50%;float:right;text-align: center;">
                    <Button v-if="isLogin" class="bg-red" style="width: 80%;" size="large" @click="addClick(1)">{{
                      $t('option.buydown')
                    }}</Button>
                    <Button v-if="!isLogin" class="bg-gray" style="width: 80%;" size="large">{{
                      $t('option.login')
                ...
                <div style="height: 20px;" class="bz_container"></div>
              </div>
            </div>
          </div>
          <!-- table -->
      ...
      columns: [
        {
          ...
          render: (h, params) => {
            return h(
              'span',
              {},
              params.row.symbol + ' - ' + self.$t('option.seriers') + params.row.optionNo + self.$t('option.period')
            )
          },
        },
        {
...
.option .buy-input .ivu-input {
  color: rgb(220, 142, 0);
  font-weight: bold;
  font-size: 20px;
  height: 35px;
}
.option .sell-input .ivu-input {
  color: #f2334f;
  font-weight: bold;
  font-size: 20px;
  height: 35px;
...
.option .ivu-input-wrapper {
  outline: none;
}

.option .ivu-input:focus,
.option .ivu-input:hover {
  box-shadow: none;
  outline: none;
}
.option .ivu-input-number-input:focus,
.option .ivu-input-number-input:hover {
  box-shadow: none;
  border-color: #41546d;
  outline: none;
}
...
.option .ivu-form-item-content input {
  padding-left: 5px;
  text-align: center;
  padding-right: 55px;
  font-size: 16px;
}
.option .ivu-form-item-content input::-webkit-input-placeholder {
  font-size: 14px;
  color: #515a6e;
  margin-bottom: 10px;
  text-align: left;
}
.option .ivu-form-item-content label.before {
  position: absolute;
  top: 4px;
  left: 10px;
  color: #7c7f82;
...
.option .ivu-form-item-content label.after {
  position: absolute;
  top: 4px;
  right: 10px;
  color: #7c7f82;
