              ...
              <div class="trade_panel trade_panel_logout" style="position: relative;">
                ...
                <div class="trade_bd">
                  <Form style="width: 100%;margin-top: 20px;">
                    <FormItem>
                      <label>{{ $t('second.transactionMode') }}</label>
                      <div class="quantity-group">
                        <div
                          v-for="item in tradeCoins"
                          :class="{ 'quantity-item': true, current: selectedTradeCoin == item }"
                          @click="selectTradeCoin(item)"
                          :key="item.id"
                        >
                          {{ item }}
                        </div>
                    ...
                    <FormItem>
                      <label>{{ $t('second.opentime') }}</label>
                      <div class="quantity-group">
                        <div
                          v-for="item in cycles"
                          :class="{ 'quantity-item': true, current: selectedCycle.cycleLength == item.cycleLength }"
                    ...
                    <FormItem>
                      <label class="before">{{ $t('second.profitability') }}</label>
                      <Input disabled placeholder="" v-model="selectedCycle.cycleRate * 100"></Input>
                      <label class="after">%</label>
                    </FormItem>
                    <FormItem>
                      <label class="before">{{ $t('second.balance') }}</label>
                      <Input disabled v-model="balance" placeholder=""></Input>
                      <label class="after">{{ selectedTradeCoin }}</label>
                    </FormItem>
                    <FormItem>
                  ...
                  <div style="width: 100%;margin-top: 20px;">
                    <div style="width: 50%;float:left;text-align: center;">
                      <Button
                        ...
                        style="width: 80%;"
                        size="large"
                        @click="addClickOrder(0)"
                        >{{ $t('option.buyup') }}</Button
                      >
                      <Button v-if="!isLogin" class="bg-gray" style="width: 80%;" size="large">{{
                        $t('option.login')
                      }}</Button>
                    </div>
                    <div style="width: 50%;float:right;text-align: center;">
                      <Button
                        ...
                        style="width: 80%;"
                        size="large"
                        @click="addClickOrder(1)"
                        >{{ $t('option.buydown') }}</Button
                      >
                      <Button v-if="!isLogin" class="bg-gray" style="width: 80%;" size="large">{{
                        $t('option.login')
                      }}</Button>
            ...
            <div class="lightning-panel" v-if="showCountDown" :style="{ background: countDownBgColor }">
              <div class="l-content">
                ...
                <p class="l-info">
                  ...
                  ><span class="l-count">{{ publishAmount | toFixedPublishAmount }} </span
                  ><span class="l-unit">{{ currentCoin.coin }}</span>
                  &nbsp;&nbsp;&nbsp;&nbsp;
                  <span class="l-txt">{{ $t('exchange.publishprice') }}：</span
                  ><span class="l-price">{{ publishPrice | toFixedPublishPrice }} </span
                  ><span class="l-unit">{{ currentCoin.base }}</span>
                </p>
                <p class="l-detail">
                  <router-link target="_blank" to="/announcement/118930">{{
                    $t('exchange.publishdetail')
                  }}</router-link>
                </p>
              </div>
            </div>
        ...
        <div class="trade_wrap">
          <div class="trade_panel trade_panel_logout">
            ...
            <div class="trade_bd">
              ...
              <div style="width: 100%;margin-top: 20px;">
                <div style="width: 50%;float:left;text-align: center;">
                  <Button v-if="isLogin" class="bg-green" style="width: 80%;" size="large" @click="addClickOrder(0)">{{
                    $t('option.buyup')
                  }}</Button>
                  <Button v-if="!isLogin" class="bg-gray" style="width: 80%;" size="large">{{
                    $t('option.login')
                ...
                <div style="width: 50%;float:right;text-align: center;">
                  <Button v-if="isLogin" class="bg-red" style="width: 80%;" size="large" @click="addClickOrder(1)">{{
                    $t('option.buydown')
                  }}</Button>
                  <Button v-if="!isLogin" class="bg-gray" style="width: 80%;" size="large">{{
                    $t('option.login')
...
<script>
import expandRow from '@components/second/expand.vue'
import Datafeeds from '@js/charting_library/datafeed/secondtrade.js'
...
export default {
    ...
    return {
      ...
      currentOrder: {
        columns: [
          ...
          {
            title: self.$t('exchange.coinSymbol'),
            render(h, params) {
              return h('span', params.row.coinSymbol)
            },
          },
          {
            title: self.$t('exchange.orderType'),
            key: 'type',
            render(h, params) {
              const row = params.row
              const type = row.type
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              return h(
                ...
                row.direction == 'BUY' ? self.$t('option.buyup') : self.$t('option.buydown')
              )
            },
          },
          {
            title: self.$t('exchange.openprice'),
            key: 'openPrice',
            render(h, params) {
          ...
          },
          {
            ...
            key: 'betAmount',
            render(h, params) {
              return h('span', self.toFloor(params.row.betAmount))
            },
          },
          {
            title: self.$t('exchange.cycle'),
            key: 'cycleLength',
            render(h, params) {
              return h('span', params.row.cycleLength + 's')
            },
          },
          {
            title: self.$t('exchange.time'),
      ...
      historyOrder: {
        ...
        columns: [
          ...
          {
            title: self.$t('exchange.closeprice'),
            key: 'closePrice',
            render(h, params) {
              return h('span', self.toFloor(params.row.closePrice))
            },
          },
          {
            title: self.$t('exchange.earn'),
            key: 'winAmount',
            render(h, params) {
              return h('span', self.toFloor(params.row.winAmount))
            },
          },
          {
  ...
  methods: {
    ...
    init() {
      ...
      if (params == undefined) {
        this.$router.push('/second/' + this.defaultPath)
      ...
      this.$store.commit('navigate', 'nav-second')
    ...
    getCycles() {
      var that = this
      let params = {}
      this.$http.post(this.host + '/second/order/cycles', params).then(response => {
        var resp = response.body
        if (resp.code == 0) {
          this.cycles = resp.data
          this.selectedCycle = resp.data[0]
        } else {
          this.$Notice.error({
            title: that.$t('exchange.tip'),
            desc: resp.message,
    ...
    selectTradeCoin(count) {
      this.selectedTradeCoin = count
      this.getWalletBalance()
    },
    getWalletBalance() {
      this.$http.post(this.host + '/second/wallet/balance/' + this.selectedTradeCoin).then(response => {
        var resp = response.body
        this.balance = resp.data
      })
    },
    selectCycle(count) {
      this.selectedCycle = count
    },
    setback() {
    ...
    updateLangData() {
      ...
      this.currentOrder.columns[1].title = this.$t('exchange.time') + 111111
      this.currentOrder.columns[2].title = this.$t('exchange.symbol')
      this.currentOrder.columns[3].title = '交易币种'
      this.currentOrder.columns[4].title = this.$t('exchange.direction')
      this.currentOrder.columns[5].title = '开仓价格'
      this.currentOrder.columns[6].title = this.$t('exchange.num')
      this.currentOrder.columns[7].title = '周期'
    ...
    addClickOrder(direction) {
      ...
      if (this.form.buy.marketAmount == 0) {
        this.$Message.error(this.$t('swap.buyamounttip'))
        return
      }
      if (this.form.buy.marketAmount > this.balance) {
        this.form.buy.marketAmount = this.balance
      }
      var that = this
      let params = {}
      ...
      this.$Spin.show()
      this.$http.post(this.host + '/second/order/add', params).then(response => {
        var resp = response.body
        if (resp.code == 0) {
          this.getCurrentOrder() //当前委托
          this.getHistoryOrder() //历史委托
    ...
    getSymbol() {
      this.$http.post(this.host + this.api.second.thumb, {}).then(response => {
        var resp = response.body
        //先清空已有数据
        for (var i = 0; i < resp.length; i++) {
          var coin = resp[i]
        ...
        require(['../../assets/js/second.js'], function(e) {
          e.clickScTab()
        })
        this.startWebsock()
    ...
    getTrade() {
      ...
      params['symbol'] = this.currentCoin.symbol
      params['size'] = 20
      this.$http.post(this.host + this.api.second.trade, params).then(response => {
        this.trade.rows = []
        var resp = response.body
        for (var i = 0; i < resp.length; i++) {
    ...
    startWebsock() {
      ...
      var that = this
      var socket = new SockJS(that.host + that.api.second.ws)
      ...
      stompClient.connect({}, function(frame) {
        ...
        if (that.isLogin) {
          ...
          //订阅委托部分交易
          stompClient.subscribe('/topic/second/order-trade/' + that.currentCoin.symbol + '/' + that.member.id, function(
            msg
          ) {
            var resp = JSON.parse(msg.body)
            that.refreshAccount()
        ...
        //订阅盘口消息
        stompClient.subscribe('/topic/second/trade-plate/' + that.currentCoin.symbol, function(msg) {
          var resp = JSON.parse(msg.body)
    ...
    getCurrentOrder() {
      ...
      var that = this
      this.$http.post(this.host + '/second/order/current', params).then(response => {
        var resp = response.body
        if (resp.data.length > 0) {
          this.currentOrder.rows = resp.data
    ...
    getHistoryOrder(pageNo) {
      ...
      params['symbol'] = this.currentCoin.symbol
      var that = this
      this.$http.post(this.host + '/second/order/history', params).then(response => {
        var resp = response.body
        let rows = []
    ...
    // },
    keyEvent(event) {
      console.log(this.baseCoinScale)
