...
public class FivePresenter implements MainContract.FivePresenter {
    ...
    @Override
    public void myInviteInfo(String token) {
        dataRepository.myInviteInfo(token, new DataSource.DataCallback() {
            @Override
            public void onDataLoaded(Object obj) {
                view.myInviteInfoSuccess((JSONObject)obj);
            }

            @Override
            public void onDataNotAvailable(Integer code, String toastMessage) {
                view.myInviteInfoFail(code, toastMessage);
            }
        });
    }
}
