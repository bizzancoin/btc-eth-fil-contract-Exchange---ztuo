...
public class SixFragment extends BaseTransFragment implements MainContract.SixView, RadioGroup.OnCheckedChangeListener, ISixContract.View, View.OnClickListener {
    ...
    public void getAccuracy(final int one, final int two, SixInfo info) {
        ...
        if (info != null) {
            ...
            //仓  一张多少usdt
            mOneYuE.setText((int) sixinfo.getShareNumber() + " "+sixinfo.getCoinSymbol());
            mTwoYuE.setText((int) sixinfo.getShareNumber() + " "+sixinfo.getCoinSymbol());
...
//            btnSale.setText(buttonTextSell);
    ...
    public void resetSymbol(Currency currency) {
        ...
        if (this.currency != null) {
            Log.i("six", "执行函数：this.currency != null");
            setCurrentSelected();
...
//            mPanJia.setText(String.valueOf(currency.getClose()));
    ...
    private void refreshgranary(Double close) {
        ...
        // 构建多仓数据
        if (data.getUsdtBuyPosition() > 0 || data.getUsdtFrozenBuyPosition() > 0) {
            double buyPL = (close - data.getUsdtBuyPrice()) * (data.getUsdtBuyPosition() + data.getUsdtFrozenBuyPosition()) * data.getUsdtShareNumber();
            //收益
            if (buyPL > 0) {
        ...
        if (data.getUsdtSellPosition() > 0 || data.getUsdtFrozenSellPosition() > 0) {
            double sellPL = (data.getUsdtSellPrice() - close) * (data.getUsdtSellPosition() + data.getUsdtFrozenSellPosition()) * data.getUsdtShareNumber();
            //收益
            if (sellPL > 0) {
    ...
    private int avaOpenSell() {
        try {
            ...
            if (data.getUsdtPattern() == "FIXED") { // 逐仓，直接usdt余额即可
                return (int) (data.getUsdtSellLeverage() * data.getUsdtBalance() / (data.getUsdtShareNumber() * currency.getClose()));
            } else {

                // 计算可用
                int buyPL = 0;
                if (data.getUsdtBuyPrice() > 0) {
                    buyPL = (int) ((currency.getClose()-data.getUsdtBuyPrice()) * (data.getUsdtBuyPosition() + data.getUsdtFrozenBuyPosition()) * data.getUsdtShareNumber());
                }
                int sellPL = 0;
                if (data.getUsdtSellPrice() > 0) {
                    sellPL = (int) ((data.getUsdtSellPrice() - currency.getClose()) * (data.getUsdtSellPosition() + data.getUsdtFrozenSellPosition()) * data.getUsdtShareNumber());
                }
                ...
                if (lossandp >= 0) {
                    ...
                    return (int) (data.getUsdtSellLeverage() * (data.getUsdtBalance() + lossandp) / (data.getUsdtShareNumber() * currency.getClose()));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private int avaOpenBuy() {
        try {
            ...
            if (data.getUsdtPattern().equals("FIXED")) { // 逐仓，直接usdt余额即可
                return (int) (data.getUsdtBuyLeverage() * data.getUsdtBalance() / (data.getUsdtShareNumber() * currency.getClose()));
            } else {
                // 计算可用
                int buyPL = 0;
                if (data.getUsdtBuyPrice() > 0) {
                ...
                if (lossandp >= 0) {
                    ...
                    return (int) (data.getUsdtBuyLeverage() * (data.getUsdtBalance() + lossandp) / (data.getUsdtShareNumber() * currency.getClose()));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
