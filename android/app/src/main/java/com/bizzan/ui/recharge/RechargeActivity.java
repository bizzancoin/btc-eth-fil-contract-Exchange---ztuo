...
public class RechargeActivity extends BaseActivity {
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        tvSubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String coinNum = etCoinNum.getText().toString().trim();
                if (TextUtils.isEmpty(coinNum)){
                    WonderfulToastUtils.showToast(getResources().getString(R.string.please_enter_the_number_of_coins));
                    return;
                }
                displayLoadingPopup();
                submit(coinNum);
    ...
    public void submit(String coinNum){
            WonderfulOkhttpUtils.post().url(UrlFactory.depositAdd())
                    .addHeader("x-auth-token", SharedPreferenceInstance.getInstance().getTOKEN())
                    .addParams("unit", coin.getCoin().getUnit())
                    .addParams("amount", coinNum)
                ...
                public void onResponse(String response) {
                    ...
                    try {
                        ...
                        if (code == 0) {
                            WonderfulToastUtils.showToast(getResources().getString(R.string.submitted_successfully));
                            finish();
                        } else {
