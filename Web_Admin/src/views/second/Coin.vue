...
import { querySecondCoinList, addSecondCoin, alterSecondCoin } from '@/service/getData'
...
export default {
  ...
  methods: {
    ...
    submit(){
      if(this.isAdd) {
        addSecondCoin(this.contractCoin).then(res => {
          if(!res.code) {
            this.$Notice.success({
                    title: "新建成功！",
                    desc: res.message,
        ...
        alterSecondCoin(this.contractCoin).then(res => {
          if(!res.code) {
            this.$Notice.success({
                    title: "修改成功！",
                    desc: res.message,
