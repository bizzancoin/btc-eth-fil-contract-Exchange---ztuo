...
- (void)senderButtonDidClick {
    // unit 代币符号
    // amount 数量
    if (self.numberTextFiled.text.floatValue <= 0) {
        return;
    }
    
    NSString *number = self.numberTextFiled.text;

    NSDictionary *dict = @{@"unit":self.unit,@"amount":number};
    
    [MineNetManager walletDeposit_addParam:dict CompleteHandle:^(id resPonseObj, int code) {
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
            
                [self.view makeToast:LocalizationKey(@"Checkout Success") duration:1.5 position:CSToastPositionCenter];
...
- (UITextField *)numberTextFiled {
    if (!_numberTextFiled) {
        ...
        _numberTextFiled.keyboardType = UIKeyboardTypeDecimalPad;
        _numberTextFiled.borderStyle = UITextBorderStyleBezel;
        _numberTextFiled.textColor = UIColor.whiteColor;
        NSAttributedString * attrstring = [[NSAttributedString alloc] initWithString:LocalizationKey(@"Please input the amount") attributes:@{NSForegroundColorAttributeName:[UIColor lightGrayColor], NSFontAttributeName:[UIFont systemFontOfSize:15]}];
        _numberTextFiled.attributedPlaceholder = attrstring;
...
- (UIButton *)senderButton {
    if (!_senderButton) {
        ...
        [_senderButton setTitle:LocalizationKey(@"submit") forState:UIControlStateNormal];
        _senderButton.titleLabel.font = [UIFont systemFontOfSize:15];
        _senderButton.backgroundColor = baseColor;
        _senderButton.layer.cornerRadius = 20;
        [_senderButton addTarget:self action:@selector(senderButtonDidClick) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:_senderButton];
    }
