...
- (void)setBgColor:(UIColor *)bgColor {
    _kLineMainView.backgroundColor = bgColor;
}
...
- (void)resetTableViewOffset{
    CGFloat oldkLineViewWidth = self.kLineModels.count * [Y_StockChartGlobalVariable kLineWidth] + (self.kLineModels.count + 1) * [Y_StockChartGlobalVariable kLineGap] + 10;
    CGFloat offset =  oldkLineViewWidth - self.scrollView.width;
...
- (void)setKLineModels:(NSArray *)kLineModels {
    ...
    CGFloat oldkLineViewWidth = self.kLineModels.count * [Y_StockChartGlobalVariable kLineWidth] + (self.kLineModels.count + 1) * [Y_StockChartGlobalVariable kLineGap] + 10;
    BOOL canMove = NO;
    if (self.scrollView.contentOffset.x == 0 || self.scrollView.contentOffset.x == oldkLineViewWidth) {
        canMove = YES;
    }
    ...
    CGFloat offset =  kLineViewWidth - self.scrollView.width;
    if (canMove) {
        self.scrollView.contentOffset = CGPointMake(offset, 0);
    }
