...
@property (nonatomic, assign,) NSInteger  oldklineMinIndex;
...
- (void)setBgColor:(UIColor *)bgColor {
    _kLineView.backgroundColor = bgColor;
    _kLineView.bgColor = bgColor;
}
...
//        NSLog(@"默认的指标是---%ld",(long)index);
        ...
        if(self.dataSource && [self.dataSource respondsToSelector:@selector(stockDatasWithIndex:)])
        {
            ...
            if(type == Y_StockChartcenterViewTypeOther)
            {
                ...
                if (self.klineMinIndex != self.oldklineMinIndex) {
                    [self.kLineView resetTableViewOffset];
                }
                self.oldklineMinIndex = self.klineMinIndex;
            }
            [self bringSubviewToFront:self.segmentView];
            stockData=nil;
        }
