...
//  EasyShowViewDemo
//
//  Created by Mr_Chen on 2019/11/24.
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        _textShadowColor = [UIColor blueColor];//阴影颜色

        _textSuperViewReceiveEvent = YES ;


        _lodingShowType = LodingShowTypeTurnAround ;
        _lodingAnimationType = lodingAnimationTypeFade ;
        _lodingTextFount = [UIFont systemFontOfSize:15];
        _lodingTintColor = [UIColor whiteColor];
        _lodingBackgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.7];
        _lodingSuperViewReceiveEvent = NO ;//控制背景是否可以点击
        _lodingShowOnWindow = YES ;
