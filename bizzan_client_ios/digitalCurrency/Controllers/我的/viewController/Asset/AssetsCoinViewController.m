...
@interface AssetsCoinViewController ()<UITableViewDelegate, UITableViewDataSource,UISearchBarDelegate>
{
    BOOL _refreshFlag;
}
...
@implementation AssetsCoinViewController
...
-(void)coinAlterView:(NSIndexPath*)indexPath withModel:(WalletManageModel *)model{
    ...
    if ([model.coin.canRecharge isEqualToString:@"1"] &&[model.coin.canWithdraw isEqualToString:@"1"]) {
        //可以充币，提币
        arrayTitle = @[LocalizationKey(@"chargeMoney") ,LocalizationKey(@"mentionMoney"),LocalizationKey(@"Transfer")];
        arrayColor = @[GreenColor,RedColor, GreenColor];
    }else if ([model.coin.canRecharge isEqualToString:@"1"] && ![model.coin.canWithdraw isEqualToString:@"1"]){
        arrayTitle = @[LocalizationKey(@"chargeMoney"),LocalizationKey(@"Transfer")];
        arrayColor = @[GreenColor, GreenColor];
    }else if (![model.coin.canRecharge isEqualToString:@"1"] && [model.coin.canWithdraw isEqualToString:@"1"]){
        arrayTitle = @[LocalizationKey(@"mentionMoney"),LocalizationKey(@"Transfer")];
        arrayColor = @[GreenColor, GreenColor];
    }else{
        arrayTitle = @[LocalizationKey(@"Transfer")];
        arrayColor = @[GreenColor, GreenColor];
    }
    [self.view createAlertViewTitleArray:arrayTitle textColor:arrayColor font:[UIFont systemFontOfSize:16] type:1  actionBlock:^(UIButton * _Nullable button, NSInteger didRow) {
        if ([button.currentTitle isEqualToString:LocalizationKey(@"chargeMoney")]) {
            //充币
            [self chargeMoneyBtnClick];
            
        }else if ([button.currentTitle isEqualToString:LocalizationKey(@"mentionMoney")]){
            //提币
            if (![self.accountInfo.kycStatus isEqualToString:@"4"]) {
                [[UIApplication sharedApplication].keyWindow makeToast:LocalizationKey(@"validateYourID") duration:1.5 position:CSToastPositionCenter];
                return ;
            }
            ...
            //取消
            [self cancelBtnClick];
        }else if ([button.currentTitle isEqualToString:LocalizationKey(@"Transfer")]){
            //划转
            [self turnOut];
        }
    }];
...
- (void)turnOut{
    ...
    _refreshFlag = YES;
    turn.unit = self.clickModel.coin.unit;
