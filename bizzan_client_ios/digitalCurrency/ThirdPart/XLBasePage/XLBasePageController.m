...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(InfoNotificationAction) name:@"PayBill" object:nil];
}
...
-(void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:@"PayBill" name:nil object:self];
}
...
-(UIScrollView *)titleBackground
{
    if (!_titleBGScroll) {
        ...
        [_titleBGScroll addSubview:line];
...
-(void)reloadScrollPage
{
    ...
    if (VCArray.count > self.selectIndex) {
...
-(void)titleButtonClick:(XLBasePageTitleButton *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(viewPagerViewController:didFinishScrollWithCurrentViewController:)]) {
        [self.delegate viewPagerViewController:self didFinishScrollWithCurrentViewController:[VCArray objectAtIndex:index]];
    }
...
-(void)scrollViewOffset:(UIButton *)button
{
    ...
    if (CGRectGetMidX(button.frame)>CGRectGetWidth(self.view.frame)) {
...
-(void)viewDidLayoutSubviews
{
    ...
    line.frame = CGRectMake(0, _titleBGScroll.frame.size.height - 1, _titleBGScroll.frame.size.width, 1);
